...
\begin{itemize}
    \item \textbf{GitHub}: piattaforma per il versionamento, la gestione dei repository, delle \textbf{issue\ap{G}}, delle \textbf{pull request\ap{G}} e dei flussi di lavoro collaborativi mediante GitHub Projects e Teams. È inoltre il punto ufficiale di archiviazione degli artefatti approvati.
...
Per il versionamento viene adottato il \textbf{Semantic Versioning (SemVer\ap{G})}.  
...
Ogni modifica nasce dall’apertura di una \textbf{issue GitHub\ap{G}}, che costituisce il punto di riferimento per la tracciabilità dell’attività.\\ Al momento della creazione, la \textbf{issue\ap{G}} deve essere dotata di un titolo descrittivo e di una spiegazione chiara dell’obiettivo. Il responsabile assegna inoltre priorità, milestone, iterazione (che corrisponde allo sprint in cui l’attività deve essere completata) e l’assegnatario, selezionato in base al ruolo che il membro ricopre in quel momento. Label aggiuntive consentono una categorizzazione accurata del lavoro.\\ La pianificazione complessiva è supportata dalle \textbf{GitHub Project Board\ap{G}}, che permettono di consultare \textbf{product backlog\ap{G}} e \textbf{sprint backlog\ap{G}} tramite viste personalizzate, semplificando la gestione dei carichi di lavoro e l’avanzamento delle attività.

\paragraph{Creazione della pull request}
Una volta completata l’attività descritta nella \textbf{issue GitHub\ap{G}}, il membro incaricato apre una \textbf{pull request\ap{G}} dal branch di feature al \texttt{branch develop}.\\ Ogni \textbf{pull request\ap{G}} deve utilizzare il template comune del gruppo, che contiene le sezioni necessarie per descrivere contesto, modifiche introdotte, issue collegate e controlli effettuati. La descrzione del templatepresente nel file \texttt{pull\_request\_template.md}, deve essere compilato e adattato ogni volta, così da velocizzare il lavoro di revisione e rendere uniformi tutte le richieste. Il membro deve indicare tramite il campo \texttt{Reviewers} il singolo membro incaricato del compito di verificare quella modifica.

\paragraph{Verifica e revisone della pull request}
All'apertura della \textbf{pull request\ap{G}} viene richiesta automaticamente la verifica ai membri del team verificatori:
\begin{itemize}
    \item se la verifica produce esito positivo, la \textbf{pull request\ap{G}} viene approvata dal verificatore incaricato;
    \item se sono presenti dei problemi o miglioramenti da applicare, il verificatore richiede delle modifiche, tramite l'opzione di \textbf{GitHub\ap{G}} \texttt{request changes}, aggiungendo un commento di quello che è il problema trovato e di una possibile soluzione se è presente. Successivamente la persona che ha aperto la \textbf{pull request\ap{G}} si impegna a eseguire le correzioni e a notificare il completamento dell'azione per avere una verifica il prima possibile.
\end{itemize}

\paragraph{Integrazione nella baseline}
Quando la \textbf{pull request\ap{G}} viene approvata del \textbf{verificatore\ap{G}}, il responsabile esegue la revisione. Si impegan ad eseguire il merge nel branch \texttt{develop}. A questo punto, la modifica entra ufficialmente in \textbf{baseline\ap{G}} e diventa parte del progetto.
...
Sul branch principale (\texttt{main}) solo i membri del team dei Responsabili possono eseguire push o merge, mentre tutti gli altri membri propongono modifiche tramite \textbf{pull request\ap{G}}. La funzione \texttt{CODEOWNERS} assegna automaticamente ai Verificatori la responsabilità di revisione dei documenti, garantendo che tutte le modifiche rispettino il workflow e i controlli qualitativi previsti.\\
...
\begin{itemize}
    \item \textbf{Inizializzazione e definizione degli ambiti}, il \textbf{responsabile\ap{G}} del progetto assieme ai membri del team identifica i requisiti per l'attività da svolgere.Si procede poi alla valutazione della fattibilità verificando disponibilità delle risorse, adeguatezza delle competenze e tempistiche.
    ...
    \item \textbf{Esecuzione e controllo}, le attività pianificate vengono attuate. Il \textbf{responsabile\ap{G}} di processo produce report periodici sullo stato di avanzamento, destinati sia all’uso interno sia all’acquirente. Il responsabile monitora l’aderenza al piano e interviene per risolvere eventuali scostamenti.
    \item \textbf{Revisione e valutazione}, perché un prodotto entri in \textbf{baseline\ap{G}}, le modifiche apportate devono essere prima verificate da un \textbf{verificatore\ap{G}} (si veda Sezione~\ref{<inserire-etichetta>}) e successivamente revisionate dal \textbf{responsabile\ap{G}}. Quest’ultimo accerta che i prodotti software e i piani risultino conformi ai requisiti stabiliti, valutando l’esito dei compiti completati e garantendo qualità e coerenza con gli obiettivi del processo.

    \item \textbf{Chiusura}, al completamento delle attività e dei prodotti previsti, si verifica che i criteri concordati con l’acquirente siano soddisfatti. Il \textbf{responsabile\ap{G}} controlla la completezza dei risultati e dei report.
\end{itemize}
...
\hline
\rowcolor{gray!60} 
{\centering \textbf{Ruoli}\par} &
{\centering \textbf{Compiti e responsabilità}\par} \\
\hline
\rowcolor{white}
\centering Responsabile & È la figura che nel momento governa il team. Identifica le attività che sono state eseguite nello spint terminato e quelle che non sono state ortate a termine. Durante l'evento di sprint planning stabilisce ed inisrisce nello Sprint Backlog le attività da eseguire nello sprint successivo, individuando i costi, i rischi e i membri del gruppo in base ai loro ruoli. Rappresenta il gruppo all'esterno, interfacciandosi con gli enti esterni quali i professori e l'azienda proponente.    \\
\hline
\rowcolor{gray!20}
...
L’organizzazione interna del gruppo (si veda sezione 4.1) è stata implementata tramite la funzione nativa di \textbf{GitHub Teams\ap{G}} di \textbf{GitHub\ap{G}}, che consente di suddividere i membri in sotto–gruppi corrispondenti ai ruoli previsti dal progetto (si veda Sezione 4.1.3). Ogni ruolo forma un team composto da uno a più membri, così da distribuire responsabilità e carico di verifica.\\
Le figure che interessano le \textbf{pull request\ap{G}} sono principlamente due:
\begin{itemize}
    \item \textbf{verificatori\ap{G}}, scelti tra i membri del team con responsabilità di verifica in quello sprint, devono controllare la correttezza formale delle modifiche;
    \item \textbf{responsabile\ap{G}}, una volta ottenuta l’approvazione dei verificatori, è incaricato di completare l’operazione di merge sul \texttt{branch main} nel rispetto delle regole di protezione del branch.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Interne}: si svolgono tra i soli membri del gruppo e hanno luogo principalmente su \textbf{Discord\ap{G}}. Ogni \textbf{Sprint\ap{G}} ha una durata di due settimane e prevede tre incontri:
    \begin{itemize}
        \item uno a fine \textbf{Sprint\ap{G}}, comprendente \textbf{Sprint Review\ap{G}}, \textbf{Sprint Retrospective\ap{G}} e \textbf{Sprint Planning\ap{G}}. In questa sede il \textbf{Responsabile} verifica lo stato delle attività concluse, analizza eventuali problemi riscontrati e pianifica il lavoro del successivo \textbf{Sprint\ap{G}};
        \item due durante lo svolgimento dello \textbf{Sprint\ap{G}}, in cui venogono trattati i temi dell'ordine del giorno e ciascun membro espone le attività completate, quelle ancora in corso e le difficoltà incontrate.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Portavoce}: identificato nel \textbf{responsabile\ap{G}}, che modera e coordina la discussione;
        \item \textbf{Redattore}: responsabile della stesura del verbale e del caricamento nel \textbf{repository\ap{G}}.
    \end{itemize}
    \item \textbf{Esterne}: coinvolgono anche figure esterne quali i docenti o l'azienda proponente \textbf{Miriade Srl}. Queste riunioni vengono svolte generalmente tramite \textbf{Google Meet\ap{G}}. La frequenza concordata è di un incontro al mese, con la possibilità di richiederne ulteriori qualora fossero necessari chiarimenti o approfondimenti specifici.
\end{itemize}
